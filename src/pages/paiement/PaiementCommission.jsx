// ============================================
// Lokatun — Page Paiement Commission Propriétaire
// ============================================
import { useState, useEffect } from 'react';
import { useParams, useNavigate, Link } from 'react-router-dom';
import api from '../../api/axios';

const PaiementCommission = () => {
  ...
  useEffect(() => {
    const charger = async () => {
      try {
        ...
        if (!r) navigate('/reservations-recues');
        setReservation(r);
      } catch {
        navigate('/reservations-recues');
      } finally {
        setChargement(false);
      }
    };
...
const handleConfirmer = async () => {
  ...
  const commission = reservation.commissionProprietaire;
    ...
    <div className="min-h-screen bg-sand-100">
      ...
      <div className="bg-secondary-500 px-6 py-4">
        <div className="max-w-2xl mx-auto flex justify-between items-center">
          <Link to="/reservations-recues" className="flex items-center gap-2 text-white hover:text-primary-300 transition">
            <span>←</span>
          ...
          <span className="text-white text-sm font-semibold">Commission propriétaire</span>
        </div>
      </div>
        ...
        {etape === 1 && (
          <div className="space-y-4">
            <div className="card p-6">
              <h2 className="text-xl font-bold text-secondary-500 mb-4">Votre commission Lokatun</h2>
              ...
              <div className="bg-orange-50 rounded-xl p-4 text-sm border border-orange-100">
                <div className="flex justify-between text-gray-600">
                  ...
                  <span>{reservation.montantBase} DT</span>
                </div>
                <div className="flex justify-between text-gray-600 mt-1">
                  <span>Vous recevez du locataire</span>
                  <span className="text-green-600 font-semibold">{reservation.montantBase} DT cash</span>
                </div>
                ...
                <div className="flex justify-between font-bold text-secondary-500 mt-2 border-t pt-2 text-base">
                  ...
                  <span>{reservation.montantProprietaire.toFixed(2)} DT</span>
                </div>
              </div>

              <div className="bg-yellow-50 rounded-xl p-4 mt-4 border border-yellow-100">
                <p className="text-yellow-700 text-sm font-semibold mb-1">⚠️ Important</p>
                <p className="text-gray-600 text-xs">
                  Vous devez payer votre commission de <strong>{commission.toFixed(2)} DT</strong> à Lokatun
                  pour pouvoir marquer la location comme terminée et débloquer les évaluations.
                </p>
              </div>
            </div>
            ...
            {/* Choix méthode */}
            <div className="card p-6">
              <h3 className="font-bold text-secondary-500 mb-4">Méthode de paiement</h3>
  ...
  {methode === 'CARTE' ? '💳 Payer via Konnect' : 'Continuer →'}
        ...
        {etape === 2 && (
          <div className="card p-6">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-500 text-sm mt-1">Commission propriétaire — Lokatun</p>
            </div>
            ...
            <div className="bg-secondary-500 rounded-xl p-5 text-white mb-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <p className="text-sm">Demandez un <strong>virement postal</strong> vers :</p>
                </div>
              </div>
              ...
              <div className="space-y-3 mt-3">
                <div className="flex gap-3 items-start">
                  ...
                  <div>
                    ...
                    <p className="text-primary-300 font-bold text-xl mt-0.5">{commission.toFixed(2)} DT</p>
                    <p className="text-blue-300 text-xs">(commission 7% uniquement)</p>
                  </div>
                </div>
                <div className="flex gap-3 items-start">
                  ...
                  <p className="text-sm">Dans la référence, notez : <strong>LOKP-{reservation.id}</strong></p>
                </div>
                <div className="flex gap-3 items-start">
            ...
            <label className="flex items-start gap-3 cursor-pointer mb-4">
              ...
              <p className="text-gray-600 text-sm">
                Je confirme avoir effectué le virement de <strong>{commission.toFixed(2)} DT</strong> vers
                le numéro postal <strong>28507819</strong> avec la référence <strong>LOKP-{reservation.id}</strong>
              </p>
            </label>
        ...
        {etape === 3 && (
          <div className="card p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-secondary-500 mb-2">Commission confirmée !</h2>
            <p className="text-gray-500 text-sm mb-6">
              Votre commission de <strong>{commission.toFixed(2)} DT</strong> est en cours de vérification.
              Vous pouvez maintenant marquer la location comme terminée.
            </p>
            ...
            <div className="bg-orange-50 rounded-xl p-4 text-left mb-6 border border-orange-100">
              ...
              <div className="space-y-1 text-sm text-gray-600">
                <div className="flex justify-between">
                  <span>Référence</span>
                  <span className="font-semibold">LOKP-{reservation.id}</span>
                </div>
                <div className="flex justify-between">
                  <span>Commission payée</span>
                  <span className="font-semibold text-primary-500">{commission.toFixed(2)} DT</span>
                </div>
                <div className="flex justify-between">
                  <span>Votre gain net</span>
                  <span className="font-semibold text-green-600">{reservation.montantProprietaire.toFixed(2)} DT</span>
                </div>
              </div>
            </div>

            <div className="flex flex-col gap-3">
              <Link to="/reservations-recues" className="btn-primary">
...
export default PaiementCommission;